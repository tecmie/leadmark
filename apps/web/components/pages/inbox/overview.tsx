...
export const InboxOverviewPage = ({
  ...
  const [inboxOverviewData, setInboxOverviewData] =
    useState<EnhancedThread[]>(data);
          ...
          try {
            ...
            if (!error && newThreadData) {
              const enhancedThread = {
                ...
                contactName:
                  `${newThreadData.contacts?.first_name ?? ""} ${newThreadData.contacts?.last_name ?? ""}`.trim(),
            ...
            prev.map((thread) =>
              thread.id === payload.new.id
                ? {
                    ...
                    ...payload.new,
                    hasNewMessages:
                      payload.new.last_updated !== payload.old.last_updated
                        ? true
